...
//		System.out.println("5. 장바구니의 항목 수량 줄이기");
		if (mCart.mCartCount == 0)
			...
			boolean quit = false;
			while(!quit) {
				System.out.println("장바구니에서 항목 수량을 줄일 도서의 ID를 입력하세요 : ");
				Scanner input = new Scanner(System.in);
				String str = input.nextLine();
				boolean flag = false;
				int numId = -1;
				...
				if (flag) {
					System.out.println("장바구니의 항목을 하나 줄이시겠습니까? Y | N ");
					str = input.nextLine();
					if (str.toUpperCase().equals("Y")) {
						System.out.println(mCart.mCartItem.get(numId).getBookID() + " 장바구니에서 도서를 하나 줄였습니다.");
						mCart.diminishCart(numId);
					}
					quit = true;
				}
...
//		System.out.println("6. 장바구니의 항목 삭제하기");
		if (mCart.mCartCount == 0)
			...
			while(!quit) {
				...
				if (flag) {
					System.out.println("장바구니의 항목을 삭제하겠습니다? Y | N ");
					str = input.nextLine();
					if (str.toUpperCase().equals("Y")) {
	...
	public static boolean isCartInBook(String bookId) {
		for (int i = 0; i < mCart.mCartCount; i++) {
			// id 가 일치하면 장바구니에 담긴 도서의 개수를 1만큼 증가시키고, TRUE를 반환 그렇지 않으면 FALSE를 반환
			if (mCart.mCartItem.get(i).getBookID().equals(bookId)) {
				mCart.mCartItem.get(i).setQuantity(mCart.mCartItem.get(i).getQuantity() + 1);
				return true;
			}
		}
		return false;
