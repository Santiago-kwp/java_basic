...
public class FactoryExample {
	...
	public void getResult(Factory mainFactory, char mainFactorySkill, IWorkingTogether partner) {
		...
		if (mainFactory instanceof CarFactory mainCarFactory) {
			System.out.println("2. 파트너 공장 ["+((CarFactory)partner).getFactoryName()+"] 협력시 추가 생산량 = "+mainCarFactory.workTogether(partner));
		} else if (mainFactory instanceof TVFactory mainTVFactory) {
			System.out.println("2. 파트너 공장 ["+((TVFactory)partner).getFactoryName()+"] 협력시 추가 생산량 = "+mainTVFactory.workTogether(partner));
		}
